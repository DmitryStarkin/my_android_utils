...
/**
 * Created by Dmitry Starkin on 11.04.2024 13:12.
 */
 ...
 *
 * usage example
 * val targetView = //ImageView for example
 * targetView.setImageDrawable(resource)
 *                                 targetView.invokeAfterLayout {
 *
 *                                     startImageRect?.let{ sourceRect ->
 *                                         TransitionBetweenTwoViews(sourceRect,  targetView.getRectSetInScreen(), (targetView.parent as View).getRectInScreen())
 *                                     }?.apply {
 *                                         targetView.animate(this, duration = 300L, offset = 100L)
 *                                     }
 *
 *
 */

class  TransitionBetweenTwoViewsRev(source: Rect, destination: Pair<Rect, Rect>, destinationParent: Rect) : AnimationSet(false) {
    init {
        ...
        addAnimation(
            TranslateAnimation(
                RELATIVE_TO_SELF, 0f, ABSOLUTE, (sourceInDestCoordinates.left.toFloat() + destImageOffset.x) /scaleX,
                RELATIVE_TO_SELF, 0f, ABSOLUTE, (sourceInDestCoordinates.top.toFloat() + destImageOffset.y)/scaleY
            )
        )
        addAnimation(
            ScaleAnimation(
