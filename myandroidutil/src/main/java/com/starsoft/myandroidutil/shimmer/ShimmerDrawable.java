...
package com.starsoft.myandroidutil.shimmer;
...
public class ShimmerDrawable extends Drawable {
    ...
    public void onBoundsChange(Rect bounds) {
        ...
        if(isCornerNotEmpty(bounds)){
            mDrawRectF.set(bounds);
            path.reset();
            path.moveTo(0, 0);
            path.addRoundRect(mDrawRectF, corner,corner, Path.Direction.CW);
        }
        updateShader();
        maybeStartShimmer();
    }
    
    private boolean isCornerNotEmpty(Rect bounds){
        
        return corner > 0 && bounds.width() >= corner * 2 && bounds.height() >= corner * 2;
    }
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        //mShaderMatrix.postScale(1.5f,1.5f);
        mShaderMatrix.postTranslate(dx, dy);
        mShimmerPaint.getShader().setLocalMatrix(mShaderMatrix);
        if(isCornerNotEmpty(mDrawRect)){
            canvas.clipPath(path);
        }
        canvas.drawPaint(mShimmerPaint);
    }
